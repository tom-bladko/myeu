...
def shortest_path(start_id, end_id, owner_tag=None, method='merchant', distances = {}, previous_nodes = {}):
    ...
    # Validate inputs
    if start_id not in province_manager.provinces:
        raise ValueError(f"Start ID {start_id} not found in provinces")
    if end_id not in province_manager.provinces:
    ...
    while priority_queue:
        ...
        current_province = province_manager.provinces[current_id]
        for neighbor_id, distance in current_province.neighbors_distance.items():
            neighbor_prov = province_manager.provinces.get(neighbor_id)
...
def _merchant_cost(current_id, neighbor_prov, owner_tag):
    ...
    roadek: RoadItem = db.roads_objs.get((current_id, neighbor_id))
    if roadek:
...
def _army_move_cost(self, current_id, neighbor_id, army_owner_tag):
    ...
    owner = self.country_manager.countries.get(army_owner_tag)
    if owner:
